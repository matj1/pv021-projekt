...
double iterace(double ***vaha, double **vysledky, double **neu, int vrstvy,
  ...
  for (int i = 0; i < vrstvy; ++i) {
    ...
    for (int k = 0; k < pocty[i + 1]) {
      ...
      vysledky[i + 1][k] =
          aktivacni(neu[i + 1][k]); // otazka vice aktivacnich funkci
    }
  }
  double max = 0;
...
int main(int argc, char **argv) {
  ...
  */
  FILE *vstup =
      fopen("../data/pv021_project/data/fashion_mnist_train_vectors.csv");

  trenink(vaha, neu, ); return 0;
}
